...
public class ConfirmServlet extends HttpServlet {
    ...
    private final String UPDATE_PAGE = "history";
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            for (String key : id.keySet()) {
                dao.checkQuantityCart(key, id.get(key));
                if (dao.checkQuantityCart(key, id.get(key)) == false) {
                    String error = "Out of order";
                    session.setAttribute("ERROR CONFIRM", error);
                    break;
                }
            }
            try {
                ...
                dao.updateHistory(name, total, daou.getAddress(name));
                int num = dao.getID();
                for (String key : id.keySet()) {
